...
namespace intersection2
{
	...
	bool intersection(const QPoint & p1,const QPoint & p2,int mis = 8)
	{
		return intersection(QPointF(p1),QPointF(p2),mis);
	}
	Result intersection(const QLine & line,const QPoint & pt,int mis = 8)
	{
		return intersection(QLineF(line),pt,mis);
	}
	Result intersection(const QList<QPoint> & pts,const QPoint & pt,bool surface = false,int mis = 8)
	{
		QList<QPointF> pts2;
