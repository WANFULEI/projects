...
static QIcon getIconToolBox(const QPixmap& pixmap, int index)
{
    ...
    QPixmap copyPix = pixmap.copy(rcImage.left(), rcImage.top(), 16, 16);
    copyPix.setAlphaChannel(QBitmap(copyPix.createMaskFromColor(QColor(0, 255, 0), Qt::MaskOutColor)));
    return QIcon(copyPix);
}
