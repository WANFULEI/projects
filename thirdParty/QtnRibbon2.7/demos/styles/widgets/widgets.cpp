...
static QString string = "<html>"
                        "<head> "
                            "<title>Combobox sample</title>"
                            "<link rel=\"STYLESHEET\" type=\"text/css\" href=\"default.css\">"
                        "</head>"
                        "<body class=\"body\">"
                        "<table width=\"260\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" align=\"left\">"
                            "<td width=\"230\" align=\"right\" valign=\"bottom\"><br><span class=\"slogan\">"
                            "<label>Correction: </label><select title=\"Correction\" name=\"correction\">"
                            "<option>with Correction</option>"
                            "<option>without Correction</option>"
                            "</select>"
                            "<br><label>Cover: </label><select title=\"Cover\" name=\"cover\" >"
                            "<option>Hardcover</option>"
                            "<option>Fenstercover</option>"
                            "</select>"
                            "<br><label>Color: </label><select title=\"Color\" name=\"color\" >"
                            "<option>Red</option>"
                            "<option>Blue</option>"
                            "<option>Silver</option>"
                            "<option>Yellow</option>"
                            "</select>"
                            "<br><label>Binding: </label><select title=\"Binding\" name=\"binding\" >"
...
void Widgets::createWebChild()
{
    QWebView* child = new QWebView;
    child->setHtml(string);
    child->setWindowTitle(tr("QWebView"));
...
void Widgets::updateStyle(QAction* act)
{
    ...
    else if (m_actOffice2007Style == act)
    {
        ...
        m_officeStyle->enableAnimation(false);
...
void Widgets::updateTheme(QAction* act)
{
    ...
    if (m_officeStyle)
    {
        ...
        if (act == m_actBlueTheme)
            styleId = Qtitan::OfficeStyle::OS_OFFICE2007BLUE;
        ...
        act->setChecked(true);
        m_officeStyle->setOptionStyle(styleId);
    }
}
...
void Widgets::about()
{
    Qtitan::AboutDialog::show(this, tr("About Qtitan Widgets"), 
        tr("OfficeStyle"), QLatin1String("2.2"));
}
