/****************************************************************************
**
** Qtitan Library by Developer Machines (Advanced RibbonBackstageView for Qt)
** 
...
void RibbonBackstageButton::paintEvent(QPaintEvent* event)
{
    ...
    option.tabStyle = d.tabStyle_;
    option.flatStyle = d.flatStyle_;
...
void RibbonBackstageViewPrivate::showChildren()
{
    for (int i = 0, count = children_.count(); count > i; i++)
        children_.at(i)->setVisible(true);
...
void RibbonBackstageViewPrivate::hideChildren()
{
    if (!ribbon_)
        return;
    ...
    {
        RibbonBar* ribbon = Q_NULL;
        QList<RibbonBar*> ribbons = qFindChildren<RibbonBar*>(widget);
        if (ribbons.count() > 0)
            ribbon = ribbons[0];
        ...
        {
            if (!(ribbon && ::isAncestor(ribbon, w)) && !::isAncestor(&p, w) && w->isVisible())
            {
                w->setVisible(false);
                children_ << w;
            }
