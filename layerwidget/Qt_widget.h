...
public:
  ...
  inline QMatrix& get_matrix() { return (*matrix); };
  void lock() { ++Locked; };
  void unlock() { if (Locked>0) --Locked; do_paint(); };
  void do_paint() { if (Locked==0) repaint(rect()); };
...
//   static Color Qt2CGAL_color(QColor c);
  ...
  QMatrix    *matrix;  // the world matrix
...
CGAL_QTWIDGET_MANIP( QColor, BackgroundColor )

// w << FillColor(c) << ... sets the fill color
CGAL_QTWIDGET_MANIP( QColor, FillColor )
